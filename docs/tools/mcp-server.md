...
本文件說明如何設定與使用 Model Context Protocol (MCP) 伺服器（MCP server）於 Gemini CLI。

## 什麼是 MCP 伺服器？

MCP 伺服器是一種應用程式，透過 Model Context Protocol（MCP）將工具與資源暴露給 Gemini CLI，使其能與外部系統及資料來源互動。MCP 伺服器扮演 Gemini 模型與本地環境或其他服務（如 API）之間的橋樑。

透過 MCP 伺服器，Gemini CLI 可以：

- **探索工具：** 透過標準化的 schema 定義，列出可用工具、其描述與參數。
- **執行工具：** 以指定參數呼叫特定工具，並接收結構化回應。
- **存取資源：** 從特定資源讀取資料（雖然 Gemini CLI 主要聚焦於工具執行）。

有了 MCP 伺服器，您可以擴充 Gemini CLI 的能力，執行超越內建功能的操作，例如與資料庫、API、自訂腳本或專業工作流程互動。

## 核心整合架構

Gemini CLI 透過內建於核心套件（`packages/core/src/tools/`）的進階探索與執行系統，與 MCP 伺服器整合：

### 探索層（`mcp-client.ts`）

探索流程由 `discoverMcpTools()` 負責協調，其步驟如下：

1. **遍歷已設定的伺服器**，來源為您的 `settings.json` `mcpServers` 設定
2. **建立連線**，使用適當的傳輸機制（Stdio、SSE 或 Streamable HTTP）
3. **透過 MCP 協定取得工具定義**，從每個伺服器取得
4. **淨化並驗證** 工具 schema，以確保與 Gemini API 相容
5. **註冊工具** 至全域工具註冊表，並處理衝突

### 執行層（`mcp-tool.ts`）

每個被發現的 MCP 工具都會包裝於 `DiscoveredMCPTool` 實例中，其負責：

- **處理確認邏輯**，依據伺服器信任設定與使用者偏好
...
- **Streamable HTTP 傳輸：** 透過 HTTP 串流進行通訊

## 如何設定您的 MCP 伺服器

Gemini CLI 會在您的 `settings.json` 檔案中，透過 `mcpServers` 設定來定位並連接 MCP 伺服器。此設定支援多個伺服器與不同的傳輸機制。

### 在 settings.json 設定 MCP 伺服器

您可以在 `settings.json` 檔案中，以兩種主要方式設定 MCP 伺服器：一是透過頂層的 `mcpServers` 物件定義特定伺服器，二是透過 `mcp` 物件進行全域設定，控制伺服器探索與執行。

#### 全域 MCP 設定（`mcp`）

您的 `settings.json` 中的 `mcp` 物件可用來定義所有 MCP 伺服器的全域規則。

- **`mcp.serverCommand`**（字串）：啟動 MCP 伺服器的全域指令。
- **`mcp.allowed`**（字串陣列）：允許連接的 MCP 伺服器名稱清單。若設定此值，僅會連接此清單中的伺服器（需與 `mcpServers` 物件中的 key 相符）。
- **`mcp.excluded`**（字串陣列）：排除的 MCP 伺服器名稱清單。此清單中的伺服器將不會被連接。
...
- **`timeout`**（數字）：請求逾時（timeout），單位為毫秒（預設：600,000ms = 10 分鐘）
- **`trust`**（布林值）：當 `true` 時，會略過此伺服器的所有工具呼叫確認（預設：`false`）
- **`includeTools`**（字串陣列）：要從此 MCP 伺服器納入的工具名稱清單。若指定，僅此清單中的工具會從此伺服器提供（白名單行為）。若未指定，預設啟用伺服器的所有工具。
...
Gemini CLI 支援使用 SSE 或 HTTP 傳輸的遠端 MCP 伺服器進行 OAuth 2.0 驗證。這可讓需要驗證的 MCP 伺服器安全存取。

#### 自動 OAuth 探索

對於支援 OAuth 探索的伺服器，你可以省略 OAuth 設定，讓命令列介面（CLI）自動發現：
...
CLI 命令列介面 (Command Line Interface) 將自動：
...
1. **初始連線嘗試** 會因 401 Unauthorized 失敗
2. **OAuth 探索** 會找到授權與權杖端點
3. **瀏覽器開啟** 讓使用者進行驗證（需要本機瀏覽器存取權）
4. **授權碼** 會交換取得存取權杖
5. **權杖會被安全儲存** 以供日後使用
6. **重新嘗試連線**，使用有效權杖即可成功
...
- **`clientId`**（字串）：OAuth client 識別碼（動態註冊時可選填）
- **`clientSecret`**（字串）：OAuth client 密鑰（公開 client 可選填）
...
你可以使用 `authProviderType` 屬性指定驗證提供者類型：

- **`authProviderType`**（字串）：指定驗證提供者。可為下列其中之一：
  - **`dynamic_discovery`**（預設）：命令列介面（Command Line Interface, CLI）會自動從伺服器發現 OAuth 設定。
  - **`google_credentials`**：命令列介面（CLI）會使用 Google Application Default Credentials（ADC）來驗證伺服器。使用此提供者時，必須指定所需的權限範圍（scopes）。
...
1. **狀態追蹤開始：** 伺服器狀態設為 `CONNECTING`
2. **傳輸方式選擇：** 根據設定屬性選擇：
   - `httpUrl` → `StreamableHTTPClientTransport`
   - `url` → `SSEClientTransport`
   - `command` → `StdioClientTransport`
3. **建立連線：** MCP client 會嘗試在設定的逾時時間內建立連線
4. **錯誤處理：** 若連線失敗則記錄錯誤，並將伺服器狀態設為 `DISCONNECTED`
...
3. **工具過濾：** 根據 `includeTools` 和 `excludeTools` 設定過濾工具
4. **名稱標準化：** 工具名稱會清理以符合 Gemini API 要求：
   - 將無效字元（非英數字、底線、點、連字號）替換為底線
   - 名稱超過 63 個字元時，會以中間替換（`___`）方式截斷

### 3. 衝突解決

當多個伺服器提供同名工具時：

1. **先註冊者優先：** 第一個註冊該工具名稱的伺服器可取得未加前綴的名稱
2. **自動加前綴：** 後續伺服器取得加上前綴的名稱：`serverName__toolName`
3. **註冊表追蹤：** 工具註冊表會維護伺服器名稱與其工具的對應關係

### 4. Schema 處理

工具參數 schema 會進行清理，以確保與 Gemini API 相容：

- **`$schema` 屬性**會被移除
- **`additionalProperties`**會被去除
- **帶有 `default` 的 `anyOf`**，其預設值會被移除（為了 Vertex AI 相容性）
- **遞迴處理**會套用於巢狀 schema
...
- **清理：** 沒有可用工具的伺服器會關閉連線
- **狀態更新：** 最終伺服器狀態設為 `CONNECTED` 或 `DISCONNECTED`
...
3. **回應處理：** 結果會同時格式化為大型語言模型 (LLM) 的 context 以及用戶顯示用的內容
...
- **`returnDisplay`：** 格式化後供用戶顯示的輸出（通常是以 Markdown 程式碼區塊呈現的 JSON）
...
- **伺服器清單：** 所有已設定的 MCP 伺服器 (MCP servers)
...
1. **檢查設定：** 確認 `command`、`args` 及 `cwd` 是否正確
2. **手動測試：** 直接執行伺服器指令，確保其可正常運作
3. **檢查相依套件：** 確認所有必要套件均已安裝
4. **檢查日誌：** 查看命令列介面 (Command Line Interface) 輸出中的錯誤訊息
5. **檢查權限：** 確保命令列介面 (Command Line Interface) 有權限執行伺服器指令
...
1. **確認工具註冊：** 確保你的伺服器確實有註冊工具
2. **檢查 MCP 協定：** 確認你的伺服器正確實作 MCP 工具清單功能
3. **檢查伺服器日誌：** 查看 stderr 輸出以找尋伺服器端錯誤
...
- **沙箱機制相容性：** 使用沙箱機制時，請確保 MCP 伺服器可在沙箱環境中運作
- **私人資料：** 使用權限範圍過廣的個人存取權杖可能導致不同儲存庫間的資訊外洩

### 效能與資源管理

- **連線持久性：** 命令列介面 (Command Line Interface) 會對成功註冊工具的伺服器維持持久連線
- **自動清理：** 未提供工具的伺服器連線會自動關閉
- **逾時管理：** 根據伺服器回應特性設定合適的逾時時間
- **資源監控：** MCP 伺服器會以獨立程序執行並消耗系統資源

### Schema 相容性

- **屬性過濾：** 系統會自動移除部分 schema 屬性（`$schema`、`additionalProperties`），以符合 Gemini API 相容性
- **名稱淨化：** 工具名稱會自動淨化以符合 API 要求
- **衝突解決：** 不同伺服器間的工具名稱衝突會自動加上前綴處理

這套完整的整合機制，讓 MCP 伺服器成為擴充 Gemini CLI 能力的強大方式，同時兼顧安全性、可靠性與易用性。

## 工具回傳豐富內容

MCP 工具不限於回傳純文字。你可以在單一工具回應中回傳多段豐富內容，包括文字、圖片、音訊及其他二進位資料。這讓你能打造功能強大的工具，在單次互動中提供多元資訊給模型。

所有從工具回傳的資料都會被處理，並作為 context 傳遞給模型的下一輪推論，讓模型能針對所提供資訊進行推理或摘要。
...
你可以在 `content` 陣列中混合使用不同內容區塊類型。支援的區塊類型包括：
...
當 Gemini CLI 收到這個回應時，會執行以下動作：

1.  擷取所有文字並將其合併為單一的 `functionResponse` 部分，提供給模型使用。
2.  將影像資料作為獨立的 `inlineData` 部分呈現。
3.  在命令列介面 (Command Line Interface) 中提供簡潔且易於理解的摘要，說明已收到文字與影像。

這讓你能夠打造能為 Gemini 模型提供豐富多模態 context 的進階工具。

## MCP Prompts as Slash Commands

除了工具之外，MCP 伺服器 (MCP servers) 也可以公開預先定義的提示（prompts），讓使用者能在 Gemini CLI 內以斜線指令（slash commands）執行。這讓你可以為常用或複雜的查詢建立捷徑，只需輸入名稱即可輕鬆呼叫。

### 在伺服器上定義 Prompts

以下是一個簡單的 stdio MCP 伺服器範例，展示如何定義 prompts：
...
這可以透過以下方式，包含在`settings.json`中的`mcpServers`下：
...
當提示（prompt）被發現後，你可以使用其名稱作為斜線指令（slash command）來呼叫它。Gemini 命令列介面 (Command Line Interface) 會自動處理參數解析。
...
當你執行此指令時，Gemini CLI 會在 MCP 伺服器上以提供的參數執行 `prompts/get` 方法。伺服器負責將參數代入提示模板（prompt template），並回傳最終的提示文字。CLI 隨後會將這個提示送交模型執行。這種方式可方便地自動化並分享常用的工作流程。

## 使用 `gemini mcp` 管理 MCP 伺服器

雖然你可以直接手動編輯 `settings.json` 檔案來設定 MCP 伺服器，Gemini CLI 也提供了一套方便的指令，讓你能以程式化方式管理伺服器設定。這些指令簡化了新增、列出及移除 MCP 伺服器的流程，無需直接編輯 JSON 檔案。

### 新增伺服器（`gemini mcp add`）

`add` 指令會在你的 `settings.json` 中設定一個新的 MCP 伺服器。根據作用範圍（`-s, --scope`），它會被加入使用者設定檔 `~/.gemini/settings.json` 或專案設定檔 `.gemini/settings.json` 檔案中。
...
這將根據範圍（`-s, --scope`），在對應的 `settings.json` 檔案中的 `mcpServers` 物件裡，找到並刪除 "my-server" 項目。
